...
class TestCaseRunner(unittest.TestCase):
  ...
  def setUp(self):
    self.environment_registry = environment_registry.new(convention.DEFAULT, create_indexed_docs(full_paths('testdata/caserunner_test.manifest.yaml')))
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_test.yaml'))))
...
class TestCaseRunnerSkipsCasesWhenSetupFails(unittest.TestCase):
  def setUp(self):
    self.environment_registry = environment_registry.new(convention.DEFAULT, create_indexed_docs([]))
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_test_setup_failures.yaml'))))
    self.results = Visitor()
...
class TestCaseRunnerCatchExceptions(unittest.TestCase):
  def setUp(self):
    ...
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_test_exception.yaml'))))
    self.results = Visitor()
...
class TestCaseRunnerNoMatchForCallTarget(unittest.TestCase):
  def setUp(self):
    self.environment_registry = environment_registry.new(convention.DEFAULT, create_indexed_docs(full_paths('testdata/caserunner_test_target.manifest.yaml')))
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_test_target.yaml'))))
    self.results = Visitor()
...
class TestCaseRunnerMatchesForCallTarget(unittest.TestCase):
  def setUp(self):
    self.environment_registry = environment_registry.new('tag:alpine', create_indexed_docs(full_paths('testdata/caserunner_test_target.manifest.yaml')))
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_test_target.yaml'))))
    self.results = Visitor()
...
class TestChdir(unittest.TestCase):
  ...
  def setUp(self):
    self.environment_registry = environment_registry.new(
        convention.DEFAULT,
        create_indexed_docs(full_paths('testdata/caserunner_test.chdir.manifest.yaml')))
    self.manager = testplan.Manager(
        self.environment_registry,
        testplan.suites_from(create_indexed_docs(full_paths('testdata/caserunner_chdir_test.yaml'))))
    self.results = Visitor()
    self.manager.accept(testplan.MultiVisitor(runner.Visitor(),
                                              summary.SummaryVisitor(verbosity=summary.Detail.FULL,
                                                                     show_errors=True)))
...
def create_indexed_docs(all_paths):
