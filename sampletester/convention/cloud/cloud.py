  ...
  def __init__(self, path: str):
    ...
    for api_dir in os.listdir(path):
      api_path = os.path.join(
          path,
          api_dir)  # ...../googleapis/artman-genfiles/python/my-language-v1
      ...
      samples_rpc_dir = os.path.join(
          api_path, 'samples', 'google', 'cloud', '_'.join(
              [api_str, version_str]), 'gapic'
      )  # ...../googleapis/artman-genfiles/python/my-language-v1/samples/google/cloud/my-language_v1/gapic
      key = 'Google.Cloud.{}.{}'.format(
          api_name, version_str)  # Google.Cloud.MyLanguage.v1
      if len(os.listdir(samples_rpc_dir)) > 0:
        self.sample_rpc_paths[key] = samples_rpc_dir

  def get_call(self, *args, **kwargs):
    service, version, rpc, sample, full_call, cli_arguments = process_call(
        args, kwargs)
    if not service:
      artifact = full_call
    else:
      ...
      if key not in self.sample_rpc_paths:
        raise ValueError(
            'could not resolve call "{}": directory for "{}" not found'.format(
                full_call, key))

      sample_path = os.path.join(self.sample_rpc_paths[key],
                                 camel_to_lower_snake(rpc), sample + '.py')
      interpreter = 'python3'
